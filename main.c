...
float film_aperture_width = 1;
float film_aperture_height = 1;
...
void draw_triangle(Win *win, v3 tp0, v3 tp1, v3 tp2, v3 color)
{
	...
	v3 normal = v3_scale(v3_noz(v3_cross(v3_sub(tp1, tp0), v3_sub(tp2, tp0))), 1);


	if (v3_dot(normal, camera_z) > 0)
		return ;
...
#define CUBES_WIDTH 90
#define CUBES_HEIGHT 90
...
int draw(void *ptr)
{
	...
	for (int z = 0; z < CUBES_HEIGHT; z++)
	{
		...
		{
			v3 p = {-2 + x * 0.4, -1 + cubes_height[z][x] * 0.5, -3 - z * 0.4};
   ...
   // draw_cube(win, light_p, V3(0, 0, 0), 0.05, V3(1, 1, 1));
